...
//	bool debug_loadGSim = false;
	...
	for(unsigned i=0; i<simParticlesVSize; i++) {
		...
		if(debug_loadGSim){
			cout << ptc << endl;
			for(unsigned k=0; k<points.size(); k++) cout << points[k]<<endl;
		}
		//    int markerstyle;
		...
		int nbofdaughters = ptc.daughterIds().size();
		const math::XYZVector& diff = points[0].positionXYZ()-points[1].positionXYZ();
		double dist = diff.R();
		// skip if nbofdaughters >0 and dist < 0.001 cm to eliminate  
		// resonances (dist = 0) and pio's (dist < 0.001)  (MDN 11 June 2008)
		if (nbofdaughters>0 && dist< 1.0E-3) {
			if(debug_loadGSim){
			 	cout << "skip PFSIM " << endl ;
			 	cout << " points [0] " << points [0].positionXYZ() << endl;
			 	cout << " points [1] " << points [1].positionXYZ() << endl;
			 	cout << " dist " << dist << endl;
